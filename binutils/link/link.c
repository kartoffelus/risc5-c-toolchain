...
void readObjHeader(ExecHeader *hdr, unsigned int inOff,
  ...
  conv4FromTargetToNative((unsigned char *) &hdr->magic);
  conv4FromTargetToNative((unsigned char *) &hdr->osegs);
  conv4FromTargetToNative((unsigned char *) &hdr->nsegs);
  conv4FromTargetToNative((unsigned char *) &hdr->osyms);
  conv4FromTargetToNative((unsigned char *) &hdr->nsyms);
  conv4FromTargetToNative((unsigned char *) &hdr->orels);
  conv4FromTargetToNative((unsigned char *) &hdr->nrels);
  conv4FromTargetToNative((unsigned char *) &hdr->odata);
  conv4FromTargetToNative((unsigned char *) &hdr->sdata);
  conv4FromTargetToNative((unsigned char *) &hdr->ostrs);
  conv4FromTargetToNative((unsigned char *) &hdr->sstrs);
  conv4FromTargetToNative((unsigned char *) &hdr->entry);
...
void readObjSegments(Module *mod, unsigned int inOff,
  ...
  for (i = 0; i < mod->nsegs; i++) {
    ...
    conv4FromTargetToNative((unsigned char *) &seg->name);
    conv4FromTargetToNative((unsigned char *) &seg->offs);
    conv4FromTargetToNative((unsigned char *) &seg->addr);
    conv4FromTargetToNative((unsigned char *) &seg->size);
    conv4FromTargetToNative((unsigned char *) &seg->attr);
  }
}
...
void readObjSymbols(Module *mod, unsigned int inOff,
  ...
  for (i = 0; i < mod->nsyms; i++) {
    ...
    conv4FromTargetToNative((unsigned char *) &symbol.name);
    conv4FromTargetToNative((unsigned char *) &symbol.val);
    conv4FromTargetToNative((unsigned char *) &symbol.seg);
    conv4FromTargetToNative((unsigned char *) &symbol.attr);
...
void readObjRelocations(Module *mod, unsigned int inOff,
  ...
  for (i = 0; i < mod->nrels; i++) {
    ...
    conv4FromTargetToNative((unsigned char *) &rel->loc);
    conv4FromTargetToNative((unsigned char *) &rel->seg);
    conv4FromTargetToNative((unsigned char *) &rel->typ);
    conv4FromTargetToNative((unsigned char *) &rel->ref);
    conv4FromTargetToNative((unsigned char *) &rel->add);
  }
}
...
void readLibHeader(LibHeader *hdr, FILE *inFile, char *inPath) {
  if (fseek(inFile, 0, SEEK_SET) < 0) {
    error("cannot seek to header in input file '%s'", inPath);
  }
  if (fread(hdr, sizeof(LibHeader), 1, inFile) != 1) {
    error("cannot read header in input file '%s'", inPath);
  }
  conv4FromTargetToNative((unsigned char *) &hdr->magic);
  conv4FromTargetToNative((unsigned char *) &hdr->omods);
  conv4FromTargetToNative((unsigned char *) &hdr->nmods);
  conv4FromTargetToNative((unsigned char *) &hdr->odata);
  conv4FromTargetToNative((unsigned char *) &hdr->sdata);
  conv4FromTargetToNative((unsigned char *) &hdr->ostrs);
  conv4FromTargetToNative((unsigned char *) &hdr->sstrs);
...
char *readLibStrings(LibHeader *hdr, FILE *inFile, char *inPath) {
  char *strs;
...
void readLibModules(int nmods, ModuleRecord *mods,
  ...
  for (i = 0; i < nmods; i++) {
    ...
    conv4FromTargetToNative((unsigned char *) &mods[i].name);
    conv4FromTargetToNative((unsigned char *) &mods[i].offs);
    conv4FromTargetToNative((unsigned char *) &mods[i].size);
    conv4FromTargetToNative((unsigned char *) &mods[i].fsym);
    conv4FromTargetToNative((unsigned char *) &mods[i].nsym);
  }
}


int needLibModule(char *fsym, unsigned int nsym) {
  int i;
  Sym *sym;
...
void readLibrary(FILE *inFile, char *inPath) {
  LibHeader hdr;
  char *strs;
  ModuleRecord *mods;
  ...
  readLibHeader(&hdr, inFile, inPath);
  strs = readLibStrings(&hdr, inFile, inPath);
  mods = memAlloc(hdr.nmods * sizeof(ModuleRecord));
  readLibModules(hdr.nmods, mods, hdr.omods, inFile, inPath);
  do {
    anyModuleExtracted = 0;
    for (i = 0; i < hdr.nmods; i++) {
      if (needLibModule(strs + mods[i].fsym, mods[i].nsym)) {
...
void readFiles(void) {
  ...
  while (file != NULL) {
    ...
    conv4FromTargetToNative((unsigned char *) &magic);
    if (magic == EXEC_MAGIC) {
...
static void writeSegment(Oseg *oseg, FILE *outFile) {
  ...
  conv4FromNativeToTarget((unsigned char *) &segment.name);
  conv4FromNativeToTarget((unsigned char *) &segment.offs);
  conv4FromNativeToTarget((unsigned char *) &segment.addr);
  conv4FromNativeToTarget((unsigned char *) &segment.size);
  conv4FromNativeToTarget((unsigned char *) &segment.attr);
